...
public class CodexSentinel : MonoBehaviour, IEnemy
{
    [Header("Enemy Stats")]
    ...
    private void Start()
    {
        if (healthBarInst.TryGetComponent<Canvas>(out canvas))
            canvas.worldCamera = Camera.main;
        healthbarExternal = healthBarInst.transform.GetChild(0).GetChild(0).GetComponent<Image>();
        ...
        StartCoroutine(nameof(LoadHealthBar));
    }
    ...
    private void Update()
    {
        ...
        if (!loadHealthBar)
        {
            healthbarExternal.fillAmount += Time.deltaTime;
            //if the healthbar isnt fully loaded, then dont even bother starting the boss fight yet
            return;
        }
    ...
    {
        healthbarExternal.fillAmount = 0f;
        yield return new WaitUntil(() => healthbarExternal.fillAmount >= 1);
        loadHealthBar = true;
    }
